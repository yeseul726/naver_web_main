...
function initSmartLevel()
{
	...
	else
	{
		...
		scriptElement.setAttribute('src', 'https://static.nid.naver.com/enclogin/ipsec.nhn?callback=setSmartLevel');
...
function ipCheckOff() {
	...
	setSmartLevel(-1);
	var scriptElement = document.createElement('script');
	scriptElement.setAttribute('src', 'https://static.nid.naver.com/enclogin/ipsec.nhn?callback=setSmartLevel&level=-1');
	document.getElementsByTagName('head')[0].appendChild(scriptElement);
}
function ipCheckOn(level) {
	...
	setSmartLevel(level);
	var scriptElement = document.createElement('script');
	scriptElement.setAttribute('src', 'https://static.nid.naver.com/enclogin/ipsec.nhn?callback=setSmartLevel&level='+level);
	document.getElementsByTagName('head')[0].appendChild(scriptElement);
}
/* cookie setting */
...
function checkAndReloadKey(api_name) {
	if (keystr == null || keystr == undefined || keystr == "") {
		var scriptElement = document.createElement('script');

		if (api_name == "key_json") {
			scriptElement.setAttribute('src', 'https://static.nid.naver.com/enclogin/keys_json_legacy.nhn?callback=keySetting');
		} else {
			scriptElement.setAttribute('src', 'https://static.nid.naver.com/loginv3/js/keys_js_legacy.nhn');
		}
		document.getElementsByTagName('head')[0].appendChild(scriptElement);
	}
}
...
function getAjaxResult(urls) {
	try {
var scriptElement = document.createElement('script');
scriptElement.setAttribute('src', 'https://static.nid.naver.com/enclogin/keys_json.nhn?callback=keySetting');
document.getElementsByTagName('head')[0].appendChild(scriptElement);
setTimeout("checkAndReloadKey('key_json')", 5000);
/*
...
function showLoginAlertSwf()
{
	showElement('div_login_no_id_message_swf');
	setTimeout("hideLoginAlertSwf()",3000);
}
function hideLoginAlertSwf()
{
	hideElement('div_login_no_id_message_swf');
}
function showLoginAlert()
{
	showElement('div_login_no_id_message');
	setTimeout("hideLoginAlert()",3000);
}
function hideLoginAlert()
{
	hideElement('div_login_no_id_message');
}
...
function getKeyByRuntimeInclude() {
	try {
		...
		keyjs.src = 'https://static.nid.naver.com/loginv3/js/keys_js.nhn';
		document.getElementsByTagName('head')[0].appendChild(keyjs);
		setTimeout("checkAndReloadKey('key_js')", 5000);
	} catch (e) {
		alert(e);
	}
}
...
function loadComplete() {
	if ( isFlashSupportBrowser() )
	{
		if (loadFail == false)
		{
			if ($("htmlarea").style.display == 'block')
			{
				hideElement('htmlarea');
			}
			if ($("flasharea").style.visibility != "visible")
			{
...
function useHtml()
{
	if (navigator.userAgent.indexOf("Chrome") != -1)
	{
		...
		$("flasharea").innerHTML="<div style='width:148px;height:67px;'>&nbsp;</div>";
	}
	else
	{
		try {
...
function setIdTextField() {
	...
	idHeight = "32";
	idMaxChars = "41";
	idTextDefault = "https://ssl.pstatic.net/static.gn/images/loginv4/bg_login_id_main_6.gif";
	idTextFont = "dotum";
	idBoldTag = "";
	idTextColor = "444444";
	idTextSize = "12";
	idBgColor = "f9fafc"
	idChangeBGColor = "f9fafc";
	idBorderSize = "1";
...
function setPwTextField() {
	...
	pwHeight = "32";
	pwMaxChars = "16";
	pwTextDefault = "https://ssl.pstatic.net/static.gn/images/loginv4/bg_login_pw_main_6.gif";
	pwTextFont = "dotum";
	pwBoldTag = "";
	pwTextColor = "444444";
	pwTextSize = "12";
	pwBgColor = "f9fafc";
	pwChangeBGColor = "f9fafc";
	pwBorderSize = "1";
...
function makeFlashObj(a, l, g, e, k, c, f, j, d) {
	...
	b
			.push('<object width="'
					...
					+ '" id="'
					+ l
					+ '" classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="https://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,0,0" >');
...
//	window.document.body.onmouseout = checkRelease;
	...
	var chk_id_login = null;
	if (navigator.userAgent.indexOf("MSIE 8") != -1 || navigator.userAgent.indexOf("MSIE 7") != -1) {
		try{
			chk_id_login = document.querySelectorAll('div.chk_id_login')[0];
		} catch(e) {}
	} else {
		chk_id_login = document.getElementsByClassName('chk_id_login')[0];
	}
	if (chk_id_login!=undefined) {
		chk_id_login.onkeydown=function(e){return div_check_id_space(e);};
	}
	
	var ip_box = null;
	if (navigator.userAgent.indexOf("MSIE 8") != -1 || navigator.userAgent.indexOf("MSIE 7") != -1) {
		try {
			ip_box = document.querySelectorAll('span.ip_box')[0];
		} catch(e) {}
	} else {
		ip_box = document.getElementsByClassName('ip_box')[0];
	}
	if (ip_box!=undefined) {
		ip_box.onkeydown=function(e){return div_check_ip_space(e);};
	}
	...
	var flashArea = $('flasharea');
	var flashObj = makeFlashObj("https://static.nid.naver.com/loginv3/commonLoginF_201505.swf", "flashlogin", flashArea.offsetWidth, flashArea.offsetHeight, "window", "null", "#f7f7f7", "false");
	flashArea.innerHTML = flashObj + flashArea.innerHTML;
	checkFlashLoad(1);
	//browser_unique_cookie_handle();
}
function div_check_ip_space(e) {
	if (window.event) {
		myKeyCode = event.keyCode;
	} else if (e.which) {
		myKeyCode = e.which;
	...
	if (myKeyCode==32) {
		$('ckb_type').checked = ! $('ckb_type').checked;
		ipCheck($('ckb_type'),e);
		return false;
	}
	return true;
}
function div_check_id_space(e) {
	if (window.event) {
		myKeyCode = event.keyCode;
	} else if (e.which) {
		myKeyCode = e.which;
	...
	if (myKeyCode==32) {
		$('chk_log').checked = ! $('chk_log').checked;
		savedLong($('chk_log'));
		return false;
	}
	return true;
